...
- `preserve_format` 默认开：保留原表格式（字体/颜色/边框/列宽），输出能直接用。
  想输出纯数值、不带样式就加 `--fast`——但注意**它基本不省时间**（实测提速 0–5%，
  在噪声范围内），别把它当加速开关用，详见下方「规模与耗时参考」。
...
4. **大表会慢，但没有硬性上限——跑之前先给用户时间预期**：耗时基本随**单元格总量**
   线性增长（实测见下方「规模与耗时参考」）。跑大任务时把 stderr 里的心跳日志转给用户，
   别让界面静默几十秒被当成卡死。
...
用户问「这么大的表能拆吗、会不会卡死」时，用这张表回答，不要含糊过去。
...
**估算口径：约 1.5 万个单元格/秒**（三档实测落在 1.4–1.7 万/秒，一致性很好）。
按此推算：10 万行 × 15 列（150 万单元格）约 100 秒。

三点要说清楚：
- **没有硬性行数/体积上限**，实测中也没出现内存问题。超大表真正的风险不是崩，
  而是**耗时长、看起来像卡死**——所以才有心跳日志。
- **瓶颈是单元格总量 + 产出文件个数**，不是格式复制。拆到几百个人（几百个输出文件）
  会比同样行数拆成 20 份慢不少。
- 正因为如此，**`--fast` 实测几乎不提速**（0–5%，在噪声范围内）。想省时间，
  更有效的办法是用 `--values` 先拆一部分，或者把源表按批次拆开跑。
