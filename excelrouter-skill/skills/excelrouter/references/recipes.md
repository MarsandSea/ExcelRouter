...
先说清楚：瓶颈是单元格总量和产出文件个数，按 **约 1.5 万个单元格/秒** 估算——
1 万行×15 列约 11 秒，5 万行约 49 秒（完整实测表见 SKILL.md「规模与耗时参考」）。

正确做法是**缩小单次任务规模**（`--fast` 不是加速开关，实测提速 0–5%）：
