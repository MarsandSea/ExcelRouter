...
先说清楚：**`--fast` 实测基本不提速**（0–5%，在噪声范围内），瓶颈是单元格总量和产出文件个数，
不是格式复制。按 **约 1.5 万个单元格/秒** 估算：1 万行×15 列约 11 秒，5 万行约 49 秒
（完整实测表见 SKILL.md「规模与耗时参考」）。

所以正确的做法是先**缩小单次任务规模**，而不是指望 `--fast`：

```bash
# ① 只拆需要的一部分取值，剩下的下次再跑
python scripts/er_split.py --input 大表.xlsx --output 拆分结果 --by 部门 --values 东区,西区

# ② 确实不想要样式时再加 --fast（作用是不复制格式，不是提速）
python scripts/er_split.py --input 大表.xlsx --output 拆分结果 --by 部门 --fast
```

跑的时候把 stderr 里的心跳/进度日志转给用户，别让界面静默几十秒被当成卡死。
